...
def complete_and_next():
    """Увеличивает счетчик и сразу переключает на новую задачу."""
    if st.session_state.current_task_index is not None:
        ...
        st.session_state.status_message = f"✅ +1 к карме! Всего: {current_count}. Следующая задача:"
    else:
        get_random_task()
...
with col_stats:
    ...
    if st.session_state.tasks_df is not None:
        st.dataframe(
            st.session_state.tasks_df.rename(columns={"task": "Задача", "count": "Счет"}),
            hide_index=True,
